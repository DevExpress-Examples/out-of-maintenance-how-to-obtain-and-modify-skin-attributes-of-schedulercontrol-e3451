...
namespace SchedulerSkinAttributes {
    public partial class SkinCustomizationForm : Form {
        ...
        private List<string> ExtractTargetSkinElementNamesHelper() {
            ...
            Type type = targetSkin.GetType();
            Hashtable targetSkinElements = (Hashtable)type.InvokeMember("Elements", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.GetProperty, null, targetSkin, new object[] { });

            foreach (DictionaryEntry dictionaryEntry in targetSkinElements) {
